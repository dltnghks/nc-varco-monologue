...
[CreateAssetMenu(fileName = "TutorialEventChannel", menuName = "Events/Gameplay Event Channel")]
public class TutorialEventChannel : ScriptableObject
{
    // 이벤트를 구독할 리스너들을 위한 Action
    // string 매개변수는 이벤트의 이름(Key)이나 추가 정보로 사용됩니다.
    public event UnityAction<ETutorialEvent> OnEventRaised;
    ...
    public void RaiseEvent(ETutorialEvent eventKey)
    {
        // 디버그 모드가 켜져있으면 로그 출력
        if (showDebugLog)
        {
